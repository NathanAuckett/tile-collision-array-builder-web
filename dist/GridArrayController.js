export default class GridArrayController {
    ...
    initialAngle = 0;
    lastAngle = 90;
    constructor(canvas, grid, outputElement) {
        this.canvas = canvas;
    ...
    calcArrayAngles() {
        ...
        for (let i = 0; i < this.grid.cellCountX - 1; i++) {
            let a = this.calcAngle(i * this.grid.cellWidth, heightArray[i] * this.grid.cellHeight, (i + 1) * this.grid.cellWidth, heightArray[i + 1] * this.grid.cellHeight);
            angleArray[i] = a;
        }
        ...
        if (this.tileSet) {
            ...
            for (let i = 0; i < this.grid.cellCountX; i++) {
                if (heightArray[i] > 0) {
                    ...
                    this.ctx.fillRect(this.grid.x1 + this.grid.cellWidth * i, this.grid.y2 - this.grid.cellHeight * heightArray[i], this.grid.cellWidth, this.grid.cellHeight * heightArray[i]);
                }
            ...
            for (var i = 0; i < this.grid.cellCountY; i++) {
                if (widthArray[i] > 0) {
                    this.ctx.fillStyle = this.fillColour;
                    this.ctx.fillRect(this.grid.x2 - this.grid.cellWidth * widthArray[i], this.grid.y1 + this.grid.cellHeight * i, this.grid.cellWidth * widthArray[i], this.grid.cellHeight);
                }
                //this.ctx.fillStyle = "white";
